...
class EmbedCommands(commands.Cog): # Cria a Classe onde ela exige que o parametro `bot` seja fornecido na sua chamada.
    ...
    @embed.command(name='edit', description='｢Discord｣ Abre um pain onde você poderá editar um Embed já existente.') # Comando que edita um Embed já existente, abre um painel com informações do embed específicado. ***OBS: O bot só poderá editar os embeds que foram enviados por ele, o mesmo serve para WebHooks.***
    @app_commands.describe(id='Forneça o id da mensagem que você deseja editar.')
    async def edit(self, interaction:discord.Interaction, id:int):
        await interaction.response.send_message("<:verifycheck:1221520019582615624> | Estamos trabalhando continuamente na criação dessa função. Ela estará disponível em um atualização futura.**",ephemeral=True) # Envia o painel no Chat que ocorreu a interação.
